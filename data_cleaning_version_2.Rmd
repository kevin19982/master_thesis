...
# the data
var_list <- c("gender", "education", "occupation", "av_income_factorized", "price_cn", "price_no_cn", "product_type", "environment", "country")

for (variable in var_list){
  print("variable: ")
  print(xtabs(~decision + variable, data = train))
  print_empty_line()
}
...
# null model
fit_0 <- glmer(as.factor(decsion) ~ 1,
               data = train, family = "binomial")



# full model
fit_1 <- glmer(as.factor(decision) ~ gender + education + occupation + 
                 av_income_factorized + price_cn + price_no_cn + product_type + 
                 environment + as.factor(country) + (1 | lfdn),
               data = train, family = "binomial")
...
sum(test$decision != preds) / nrow(test)
...
truth_fc <- factor(test$decision, labels = c("no_cn", "cn"))


ConfusionTableR::binary_visualiseR(train_labels = preds_fc,
