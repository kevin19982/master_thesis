...
```{r select observations based on completion time}
...
```{r recoding outcome variables to get them into the right format}
...
# it's a small land)
unique(data_recoded$country)
unique(data_recoded$Country_Name)
...
```{r create table with demographics}
library(sjPlot)
library(gtsummary)
library(gt)
...
table_one <- tbl_summary(data_recoded,
            ...
            label = c(age ~ "Age, median (SD)",
                      ...
                      "environment" ~ "View of environmental issues, n (%)"),
            missing = "no") %>% 
...
# ggplot(data_recoded %>% filter(gender %in% c("Female", "Male")),
#        aes(x = age)) +
#   geom_density(aes(fill = gender), alpha = 0.8) +
#   scale_fill_ghibli_d("LaputaMedium", direction = -1) +
#   theme_bw()
...
# calcualte dissimilarity matrix
library(cluster)
...
# library(fpc)
# 
# c_stats_table <- function(dist, tree, k){
#   cluster_assess <- c("cluster_number", "n", "within_cluster", "average_within", "average_between", "wb_ratio", "dunn_2", "avg_silwidth")
#   cluster_size <- c("cluster_size")
#   stats_names <- c()
#   row_cluster <- c()
#   
#   output_stats <- matrix(ncol = k, nrow = length(cluster_assess))
#   cluster_sizes <- matrix(ncol = k, nrow = k)
#   
#   for (i in c(1:k)){
#     row_cluster[i] <- paste("Cluster-", i, " size")
#   }
#   
#   for (i in c(2:k)){
#     stats_names[i] <- paste("Test", i - 1)
#     
#     for (j in seq_along(cluster_assess)){
#       output_stats[j, i] <- unlist(cluster.stats(d = dist, clustering = cutree(tree, k = i))[cluster_assess])[j]
#     }
#     
#     for (d in 1:k){
#       cluster_sizes[d, i] <- unlist(cluster.stats(d = dist, clustering = cutree(tree, k = i))[cluster_size])[d]
...
# 
# 
# stats_df_divisive <- c_stats_table(gower_distance, divisive_clustering, 5)
# stats_df_divisive
...
# optimize number of classes based on AIC, BIC an max log likelihood
...
# 
# 
# # apparently not good for mixed data
# data_cluster_scaled <- scale(data_cluster)
# 
# som_grid_1 <- somgrid(xdim = 5, ydim = 5, topo = "hexagonal")
# 
# som_model_1 <- som(data_cluster_scaled, grid = som_grid_1)
# 
# plot(som_model_1, type = "codes")
# 
# plot(som_model_1, type = "mapping")
...
```{r cluster variables}
# nice way of showing relationships of variables
# plot(hclust(dist(abs(cor(na.omit(data_cluster))))))
# 
# print(cor(na.omit(data_cluster)))
# print(dist(abs(cor(na.omit(data_cluster)))))
# 
# 
# # correlation matrix for categorical data does not make sense does it?
# # would it work if it's binarized?
# # the data does not have to be normalized, right?
# 
...
```{r check relationships of variables}
# chi-squared
...
data_lf$decision <- ifelse(data_lf$decision == 2, 1, 0)
...
```{r check representativeness of data}
...
train_size <- floor(0.85 * length(individuals))
test_size <- (length(individuals) - train_size)
...
test_idx <- individuals[which(!individuals %in% train_idx)]
...
# # split data into train and test set based on observations overall 
# sample_size <- floor(0.75 * nrow(data_model))
# 
# set.seed(100)
# train_idx <- sample(seq(1, nrow(data_model), 1), size = sample_size)
# 
# train_alter <- data_model[train_idx, ]
# test_alter <- data_model[-train_idx, ]
...
set.seed(100)
train_rose <- ROSE(as.factor(decision) ~ lfdn + age + as.numeric(gender) + 
...
fit_1_2 <- glmer(decision ~ gender + education + occupation + 
                   av_income_factorized + price_cn + price_no_cn + 
                   product_type + environment + age + I(age^2) + (1 | lfdn),
...
# another model
fit_1_3 <- glmer(decision ~ education + occupation + 
                   av_income_factorized + price_cn + price_no_cn + 
                   product_type + environment + (1 | lfdn),
                 data = data_model, family = "binomial")
...
# as a sanity check, let's try a model without mixed effects
fit_fe <- glm(decision ~ gender + education + occupation + 
                av_income_factorized + price_cn + price_no_cn + product_type + 
                environment + age,
              data = data_model, family = "binomial"(link = "logit"))
...
anova(fit_1, fit_fe)
# mixed-effects work better in this case (as theoretically derived by the
# data having a panel-data structure)

# I decide for model specification 1, this will be the features used for
...
car::vif(fit_1)
# when adjusting for the degrees of freedom, all regressors seem to be fine
# and not exhibit too much multicollinearity

# try an updated model
fit_vif_1 <- glmer(decision ~ gender + education + occupation + 
                     av_income_factorized + price_no_cn + 
                     environment + age + (1 | lfdn),
                   data = data_model, family = "binomial"(link = "logit"))

summary(fit_vif_1)

anova(fit_1, fit_vif_1)

car::vif(fit_vif_1)


fit_vif_2 <- glmer(decision ~ gender + education + 
                     price_no_cn + 
                     environment + age + (1 | lfdn),
                   data = data_model, family = "binomial"(link = "logit"))
...
anova(fit_1, fit_vif_2)

car::vif(fit_vif_2)

# excluding variables with a somewhat higher base VIF leads to a significantly
# worse model going by AIC, BIC, log likelihood, and residual deviance
# when accounting for the degrees of freedom all initial variables exert
...
# # pseudo-R^2 of fixed-effects model
# # (does not work for mixed effects)
# calculate_lr_r2 <- function(logistic_model){
#   dev <- logistic_model$deviance
#   null_dev <- logistic_model$null.deviance
#   num <- length(logistic_model$fitted.values)
#   r2_l <- 1 - dev / null_dev
#   r2_cs <- 1 - exp(-(null_dev - dev) / num)
#   r2_n <- r2_cs / (1 - (exp(-(null_dev / num))))
#   cat("Pseudo R^2 for the model\n")
#   cat("Hosmer and Lemeshow R^2: ", round(r2_l, 4), "\n")
#   cat("Cox and Snell R^2: ", round(r2_cs, 4), "\n")
#   cat("Nagelkerke R^2: ", round(r2_n, 4), "\n")
# }
# 
# calculate_lr_r2(fit_fe)
...
# fit_test_1 <- glmer(decision ~ gender + age + (1 | lfdn),
#                     data = data_model,
#                     family = "binomial"(link = "logit"))
# 
# library(car)
